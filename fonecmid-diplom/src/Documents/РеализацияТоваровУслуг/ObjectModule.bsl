...
Процедура ВКМ_ВыполнитьАвтозаполнение() Экспорт
    ...
    Запрос.Текст = 
        "ВЫБРАТЬ
        |	СУММА(ВКМ_КоличествоЧасовОборот) КАК КоличествоЧасов,
        |	СУММА(ВКМ_СуммаКОплатеОборот) КАК СуммаКОплате
        |ИЗ
        |	РегистрНакопления.ВКМ_ВыполненныеКлиентуРаботы.Обороты(&НачалоМесяца, &КонецМесяца,, ВКМ_Договор = &Договор) КАК
        |		Обороты";
    ...
    Если Выборка.Следующий() И ЗначениеЗаполнено(Выборка.КоличествоЧасов) Тогда
        ...
        Строка.Цена         = Выборка.СуммаКОплате / Выборка.КоличествоЧасов;
    КонецЕсли;
